  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 animate-fade-in">
      <div className="text-center mb-16">
        <h1 className="text-4xl md:text-5xl font-serif mb-4">{catParam || collectionParam || 'Nossas Peças'}</h1>
      ...
      <div className="flex flex-col md:flex-row gap-12">
        <aside className="w-full md:w-64 space-y-10">
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li><Link href="/catalog" className={`text-xs uppercase tracking-widest ${!catParam ? 'text-[#D5BDAF] font-bold' : 'text-gray-600'}`}>Tudo</Link></li>
              {settings.categories.map(c => (
                <li key={c}>
                  <Link href={`/catalog?cat=${c}`} className={`text-xs uppercase tracking-widest ${catParam === c ? 'text-[#D5BDAF] font-bold' : 'text-gray-600'}`}>{c}</Link>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="flex-grow">
          {filteredProducts.length === 0 ? (
            <div className="text-center py-20 text-gray-400 italic">Nenhum detalhe encontrado.</div>
          ) : (
...
export default function Catalog() {
  return (
    <Suspense fallback={<div className="p-20 text-center">Carregando catálogo...</div>}>
      <CatalogContent />
    </Suspense>
  );
}