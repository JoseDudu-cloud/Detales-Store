        ...
        <aside className="w-full md:w-64 space-y-10">
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              {/* Fix: Explicitly typing category map variable to avoid 'unknown' type error */}
              {settings.categories.map((c: string) => (
                <li key={c}><Link href={`/catalog?cat=${c}`} className={`text-xs uppercase tracking-widest ${cat === c ? 'text-[#D5BDAF] font-bold' : 'text-gray-600'}`}>{c}</Link></li>
              ))}
            </ul>
          </div>
        </aside>
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="group">
              <Link href={`/product/${product.id}`}>
                <div className="aspect-[3/4] overflow-hidden rounded-3xl mb-4 bg-gray-50 border border-gray-50">
              ...
              <div className="text-center">
                ...
                <p className="font-serif font-bold">R$ {product.price.toFixed(2)}</p>
                <button onClick={() => addToCart(product.id)} className="mt-4 bg-gray-50 p-3 rounded-full hover:bg-black hover:text-white transition">
                  <ShoppingBag size={16} />
                </button>
              </div>
            </div>
