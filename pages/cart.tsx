  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-6">
        ...
        <h2 className="text-3xl font-serif mb-6">Sua sacola está vazia</h2>
      ...
      <div className="space-y-8">
        {cartItems.map(item => (
          <div key={item.productId} className="flex items-center space-x-6 pb-6 border-b border-gray-50">
            ...
            <button onClick={() => removeFromCart(item.productId)} className="text-gray-300 hover:text-red-500"><Trash2 size={20} /></button>
          </div>
        ))}
      </div>
      <div className="mt-12 p-8 bg-[#FAF7F2] rounded-3xl text-center">
        <p className="text-2xl font-serif mb-8">Total: R$ {total.toFixed(2)}</p>
        <button onClick={handleCheckout} className="bg-[#25D366] text-white px-12 py-5 rounded-full font-bold uppercase tracking-widest text-xs shadow-xl flex items-center justify-center mx-auto space-x-3">
          <Send size={18} />
